**流量转发面板产品需求草案 · v0.2 · 2026-09-20**

状态：需求梳理，尚未开始产品代码实现。基础要求来自用户描述，功能补充来自 [ny 面板调研](research/ny-panel-observations-2026-09-20.md)。新增建议与参考产品现有行为均需作为需求评审输入，不能视作已经获准实现的全部范围。

产品包含管理员后台、用户前台、转发节点服务和服务器探针。管理端负责配置与运营；节点执行转发、策略与计量；两端界面按权限查看数据。建议用户入口为 `/`、管理入口为 `/admin`；用户原话中的 `/admin` 描述的是参考站点，不能视为已经确认的新产品路由规范。

本次复核结论：8 项原始要求均已收录，上一版仍缺少跨模块规则、失败恢复与可执行验收。v0.2 保留 R01–R32，新增 R33–R48，并展开页面、角色、协议、数据、账务、运行维护和异常行为。模块清单已覆盖当前已知范围；首批支付渠道、协议定义、计费与规模等仍有待决定事项，不能据此宣称需求已完全定稿。
...
| M06 | SQLite、PostgreSQL、MySQL | 内置库暂按 SQLite 理解；选择任一支持库部署后通过相同核心业务验收与迁移测试 |
| M07 | 主要功能提供管理 API | 版本化文档、统一错误、分页筛选、权限、服务账号/API Token、限流和审计 |
| M08 | 添加设备组时可选择屏蔽协议 | 新增和编辑均提供控件；显示适用层级、识别能力与限制；验证配置确实下发并生效 |

“WS 隧道加密”需明确是 WSS 还是 WS 内的额外加密；HTTP 隧道需明确 CONNECT 或自定义封装。未明确前保留这两项，不将其擅自合并或删除。

**建议补充的功能清单**

优先级是评审建议：P0 为形成核心运营闭环所需，P1 为运营与可靠性增强，P2 为可选高级能力。以上 M01–M08 仍是明确要求，不因本表优先级被移出范围。
...
| 渠道 | 需求来源 | 当前状态 |
|---|---|---|
| EPay / 易支付 | 用户明确指定；参考面板有配置入口 | 必须纳入，具体兼容版本和签名方式待选定 |
| EPUSDT、TokenPay、Cyber、Cryptomus | 参考面板表单 | 建议列入适配候选，尚未验证真实支付 |
...
1. 钱包模式：用户充值 → 支付确认 → 钱包账本入账；用户另行购买套餐 → 扣余额并记录购买 → 发放权益。套餐直接支付模式若纳入，则由该笔支付发放对应权益，不同时自动增加可消费的钱包余额。
...
| WS 隧道 | WebSocket 承载，本身不含 TLS；用户提出的额外加密需求继续保留 | path、Host、连接端点、鉴权、超时、帧/消息限制 | 是否需要额外加密、密钥协商及其与 WSS 的区别 |
| WSS 隧道 | WebSocket 经 TLS 传输，两层配置均有效 | WS 参数与 TLS 证书/SNI/校验参数 | 代理/CDN 的升级连接、超时和端口兼容；UDP 封装 |
| HTTP 隧道 | 明确 CONNECT 或其他 HTTP 封装；是否额外套 TLS 单列 | HTTP 版本、方法/路径、鉴权、代理与超时 | HTTP/1.1 与 HTTP/2 范围、明文/HTTPS、UDP 方式 |
...
日付/月付需明确按自然日/月还是固定时长；月末、闰年、夏令时、提前续费、到期宽限、跨套餐升级/降级、可叠加流量包消耗顺序、到期时间和未用流量处理都进入 D06 决策。不能把参考站点“续费重置”的行为默认套用到新产品。
...
内置数据库暂按 SQLite，其确切选择仍属 D10。SQLite、PostgreSQL、MySQL 三种部署都应覆盖同样的账户、订单、规则和计量功能；支持的具体数据库版本与系统架构要发布兼容列表。SQLite 的写并发和单实例边界应如实说明，不能把使用共享文件等同于已经支持多副本服务。
...
当前没有用户确认的规模指标，不填写虚构的“已达标”数字。以下值在性能验收前必须确定，基准测试同时记录 CPU、内存、带宽、网络时延、数据库和构建版本。服务启动和页面可打开不能代替这些验收。

| 指标 ID | 要填写的目标 | 验收方法 |
|---|---|---|
| N01 | 注册/同时在线节点数，用户数，规则总数与单机规则数 | 指定容量数据下执行列表、创建、配置下发和状态订阅 |
| N02 | 每节点及总吞吐、并发连接、每秒新连接、CPU/内存上限 | 对已选协议和 TCP/UDP 分别测，记录额外开销与失败率 |
| N03 | API P95/P99 响应时间、错误率与并发量 | 分页/过滤/大批量任务同时运行下统计，排除网络条件差异 |
| N04 | 指标采样/展示延迟、断线判定、重连、撤权与配置生效上限 | 从操作或节点事件时间到界面/节点实际变化测量 |
| N05 | 流量和限速误差、重复/迟到上报处理时限 | 已知字节量与受控时钟/重启/重放的对账实验 |
| N06 | 监控、账务、审计、任务和备份的保留期与容量预算 | 持续写入/聚合/清理后验证可查询范围及磁盘使用 |
| N07 | 服务可用性目标、可丢数据时间 RPO、恢复时间 RTO | 停服务、数据库恢复和故障演练；单机/多副本部署分别声明 |
| N08 | 支持的操作系统/架构、浏览器及移动端范围 | 按支持矩阵安装、升级并执行关键页面与转发场景 |

**待决定事项与建议口径**

这些是需求决定，不等同于必须再次申请权限。后续可按已明确的用户偏好确定；尚未答复时保留为待定，不能把建议写成已经批准的实现或验收值。

| 决策 ID | 待决定内容 | 建议起点与影响 |
|---|---|---|
| D01 | 自有 Agent/协议或兼容 ny；是否迁移旧站数据 | 建议先自有协议与契约；兼容和迁移另做验证，不通过网页抓包承诺兼容 |
| D02 | 第一批支付平台、版本、地区、币种、商户方式 | EPay 必须支持；其余按实际商户及用户需求排期，逐渠道验收 |
| D03 | 钱包、套餐直付或两者 | 建议先钱包充值再购买，是否提供直付单列；无论选择哪种都不能重复入账与发放 |
| D04 | WS 加密含义、HTTP 封装、TCP/UDP、证书/鉴权和 Mux 范围 | 四类必需协议保留；提交每类明确协议矩阵后实现，不用探针 WSS 代替 |
| D05 | 流量方向、倍率组合、限速层级、连接/IP 计数与精度 | 原始计量与计费分开、保留策略版本；具体公式由运营口径决定 |
| D06 | 续费时间、月末规则、升级降级、叠加、消费顺序、到期/超额处理 | 建议提前续费避免无提示损失既有权益；是否累加流量等仍需明确 |
| D07 | 普通用户探针范围与 IP/硬件可见性 | 建议只显示授权组及明确开放字段；公共探针视图作为独立候选 |
| D08 | 屏蔽枚举、上下级策略、冲突优先级、未知/加密流量处理 | 建议用户只能收紧组策略；不明协议先标记未知，不能伪称识别成功 |
| D09 | 面板失联时的节点策略、缓存期限、撤销和超额误差上限 | 建议定义有期限的缓存权益与一致性边界，不能同时承诺无限离线和即时撤销 |
| D10 | 内置数据库选择、数据库版本、单机/多副本及跨库迁移 | 建议 SQLite 内置、PG/MySQL 可选；跨库迁移与高可用单独立项 |
| D11 | 规模、性能、保留期和 RPO/RTO 数值 | 补齐 N01–N08 后建立可复现基准；当前不宣称无限规模 |
| D12 | 首版边界：自带出口、链式、反向、端口共享、返佣与远程终端 | 依据上表优先级选择；所有 M 项仍保留，P2 不自动进入首版 |
| D13 | 账号恢复通道、MFA、细分管理角色、用户注销与帮助方式 | 基础恢复和权限必须存在；邮件服务、工单及具体角色模板另定 |
| D14 | 站点时区/币种、金额舍入、退款/拒付与账务保留规则 | 一站一致展示，历史订单留快照；渠道精度不能仅按 UI 小数位截断 |
| D15 | 支持的 OS/架构、容器/原生/离线安装、升级方式及路由/域名 | 建议明确首批 Linux 架构与两个面板入口；不默认支持所有平台 |

目前可用本文拆分模块、评审依赖和定义测试。涉及 D02–D06、D08–D11、D14 等的实现不能凭默认猜测承诺最终行为。协议执行与服务端证据仍按 [下一轮验证计划](research/server-validation-plan.md) 补充；对应需求的验收场景见 [验收场景](acceptance-scenarios.md)。
